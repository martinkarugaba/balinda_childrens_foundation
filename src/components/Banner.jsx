...
const Banner = () => (
  <div className="-mt-[6rem] flex h-auto w-[100%] items-center justify-center px-[7rem] text-off-white">
    {bannerData.map((item) => {
      const {
        id, icon, heading, title, backgroundColor,
      } = item;
      return (
        <div
          key={id}
          className={`bg-${backgroundColor} md:flex-1 drop-shadow-md font-secondary rounded-lg cursor-pointer px-[3.5rem] py-[3.5rem]`}
        >
